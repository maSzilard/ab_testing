(function () {
    ...
    function initCollapse() {
        if (isInitialized || document.querySelector('.header-toggle-arrow')) {
            // Already initialized — ensure header is visible in case VWO re-ran the script
    ...
    // Inject CSS so it survives VWO re-execution (external CSS file gets removed on re-run)
    if (shouldRunOnMobile()) {
        const style = document.createElement('style');
        style.textContent = '#main-content header.woocommerce-products-header h1 { margin-bottom: 0; }';
